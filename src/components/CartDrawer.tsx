    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-card border-border">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="text-card-foreground font-display text-2xl">Shopping Cart</SheetTitle>
            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 rounded-lg bg-secondary/50">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium text-sm text-card-foreground truncate">{item.product.node.title}</h4>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-accent">{items[0]?.price.currencyCode || '$'} {totalPrice.toFixed(2)}</span>
                </div>
